...
@pytest.mark.parametrize("multiProcess, block", [(False, True), (True, True)])
def test_variable_length_string(multiProcess, block):
    ...
    # Add test for git issue #31, metadata variables are not being corectly read as variable length strings
    credentials = {"profile":"default"}
    h5obj = h5coro.H5Coro(url, s3driver.S3Driver, errorChecking=True, verbose=False, credentials=credentials, multiProcess=multiProcess)
    promise = h5obj.readDatasets(datasets, block=block, enableAttributes=False)

    # Extract metadata
    sliderule_metadata = promise["metadata/sliderule"]
    print(f"sliderule_metadata: {sliderule_metadata}")
    ...
    assert sliderule_metadata == exp_json_string
